...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- octotask.dev: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via octotask.dev bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const githubToken =
      ((context?.cloudflare?.env || process.env) as any)?.GITHUB_BUG_REPORT_TOKEN ||
      process.env.GITHUB_BUG_REPORT_TOKEN;
    const targetRepo =
      ((context?.cloudflare?.env || process.env) as any)?.BUG_REPORT_REPO ||
      process.env.BUG_REPORT_REPO ||
      'stackblitz-labs/octotask.dev';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'octotask.dev-bug-reporter',
    });
